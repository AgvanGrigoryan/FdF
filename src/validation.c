...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/14 18:14:17 by aggrigor          #+#    #+#             */
/*   Updated: 2024/03/22 20:52:15 by aggrigor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsing_first_line(int fd, t_vars *vars)
{
	...
	vars->x = ft_array_len(vars->row_splited);
	if (vars->x == 0)
		return (pfree("Invalid map size\n", vars, 1));
	vars->y = 1;
	vars->map = malloc(sizeof(int *) * (vars->y + 1));
	if (vars->map == NULL)
		return (pfree("Allocation failed\n", vars, 1));
...
int	parsing_other_lines(int fd, t_vars *vars)
{
	...
	while (1)
	{
		...
		if (row_length != vars->x)
			return (pfree("Invalid map size\n", vars, 1));
		vars->y++;
		if (extend_map(vars) == 1)
			return (pfree("Allocation failed\n", vars, 1));
		vars->map[vars->y - 1] = convert_line(vars);
		if (vars->map[vars->y - 1] == NULL)
			return (pfree("Allocation failed\n", vars, 1));
